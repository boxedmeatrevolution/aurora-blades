...
onready var particles_material := self.process_material as ParticlesMaterial

func _ready():
	var viewport_rect := self.viewport.get_visible_rect()
	var width := 0.5 * PADDING_HORIZONTAL_FACTOR * viewport_rect.size.x
	self.particles_material.emission_shape = ParticlesMaterial.EMISSION_SHAPE_BOX
	self.particles_material.emission_box_extents = Vector3(width, 0.0, 0.0)
